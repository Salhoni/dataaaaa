...
@st.cache_data  # Adding cache for performance
def load_data(file):
    try:
...
def find_correlations(data):
    try:
        ...
        st.error(f"Error finding correlations: {e}")
...
def advanced_stats(data):
    try:
        ...
        st.error(f"Error performing advanced statistical tests: {e}")
...
if uploaded_file is not None:
    ...
    if data is not None:
        ...
        if st.sidebar.checkbox("Apply Category Filter"):
            ...
            categories = st.sidebar.multiselect("Select Categories", options=data[cat_col].
